...
public class ChoicePanel : MonoBehaviour
{
    ...
    private const float BUTTON_HIGHT_PER_LINE = 50;
    private const float BUTTON_HIGHT_PADDING = 20;
    ...
    private IEnumerator GenerateChoices(string[] choices)
    {
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            ...
            else
            {
                ...
                Button newButtton = newButtonObject.GetComponent<Button>();
                TextMeshProUGUI newTitle = newButtton.GetComponent<TextMeshProUGUI>();
                LayoutElement newLayout = newButtton.GetComponent<LayoutElement>();

                choiceButton = new ChoiceButton { button = newButtton, layout = newLayout, title = newTitle };
        ...
        foreach (var button in buttons)
        {
            // buttons[i].title.ForceMeshUpdate();
            int lines = button.title.textInfo.lineCount;
            Debug.Log(lines);
            button.layout.preferredHeight = BUTTON_HIGHT_PADDING + (BUTTON_HIGHT_PER_LINE * lines);
        }
    }
