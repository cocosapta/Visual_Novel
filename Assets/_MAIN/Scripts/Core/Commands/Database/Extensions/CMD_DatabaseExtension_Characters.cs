...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Characters : CMD_DatabaseExtension
    {
        ...
        new public static void Extend(CommandDatabase database)
        {
            ...
            CommandDatabase spriteCommands = CommandManager.instance.CreateSubDatabase(CommandManager.DATABASE_CHARACTER_BASE);
        ...
        public static IEnumerator HighlightAll(string[] data)
        {
            List<Character> characters = new List<Character>();
            bool immediate = false;
            bool handleUnspesificedCharacters = true;
            ...
            parameters.TryGetValue(new string[] { "-o", "-only" }, out handleUnspesificedCharacters, defaultValue: true);

            foreach (Character character in characters)
                character.Highlight(immediate: immediate);

            if (handleUnspesificedCharacters)
            {
            ...
            if (!immediate)
            {
                CommandManager.instance.AddTerminationActionToCurrentProcess(() =>
                {
                    ...
                    if (!handleUnspesificedCharacters) return;

                    foreach (var character in unspecifiedCharacters)
                        character.UnHighlight(immediate: true);
                });

                while (characters.Any(c => c.isHighlighting) || (handleUnspesificedCharacters && unspecifiedCharacters.Any(uc => uc.isUnHighlighting)))
                    yield return null;
            }
        }
        public static IEnumerator UnhighlightAll(string[] data)
        {
            ...
            if (!immediate)
            {
                ...
                while (characters.Any(c => c.isUnHighlighting) || (handleUnspesificedCharacters && unspecifiedCharacters.Any(uc => uc.isHighlighting)))
                    yield return null;
            }
        }

        #region BASE CHARCTER COMMANDS
        ...
        public static void SetPriority(string[] data)
        {
            ...
            character.SetPrioriry(priority);
        }
